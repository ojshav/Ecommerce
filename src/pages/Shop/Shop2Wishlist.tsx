...
import Shop2ProductCard from '../../components/shop/shop2/Shop2ProductCard';
...
const Shop2Wishlist: React.FC = () => {
  const { wishlistItems: shop2WishlistItems, isLoading: loading } = useShopWishlistOperations(SHOP_ID);
    ...
    return (
      <div className="container mx-auto px-4 py-6 flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-500">Loading your Shop 2 wishlist...</p>
        </div>
      </div>
    );
  }

  if (!shop2WishlistItems.length) {
    return (
      <div className="container mx-auto w-full py-6">
        <div className="flex flex-col max-w-[1440px] items-center justify-center min-h-[60vh] text-center bg-white rounded-lg shadow-sm p-8 mx-auto">
          ...
          <h2 className="text-2xl font-semibold mb-3">Your Shop 2 wishlist is empty</h2>
          <p className="text-gray-500 mb-8 max-w-md">
            Save your favorite Shop 2 products to your wishlist to keep track of items you love and want to purchase later.
          </p>
          <button
            onClick={() => navigate('/shop2')}
    ...
    <div className="container w-full mx-auto px-4 py-6">
      <div className="bg-white max-w-[1440px] rounded-lg shadow-sm p-6 mb-8 mx-auto">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-semibold mb-1">My Shop 2 Wishlist</h1>
            <p className="text-gray-500">{shop2WishlistItems.length} {shop2WishlistItems.length === 1 ? 'Item' : 'Items'}</p>
          </div>
          <button
            onClick={() => navigate('/shop2')}
          ...
          {shop2WishlistItems.map((item) => (
            <div key={item.wishlist_item_id} className="transform transition-all duration-300 hover:scale-[1.02]">
              <Shop2ProductCard
                ...
                discount={item.product.discount_pct}
                onClick={() => navigate(`/shop2-productpage?id=${item.shop_product_id}`)}
              />
            </div>
          ))}
        </div>
