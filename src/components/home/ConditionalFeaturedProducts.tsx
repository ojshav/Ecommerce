...
const ConditionalFeaturedProducts: React.FC = () => {
  ...
  useEffect(() => {
    const checkForProducts = async () => {
      try {
        const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/featured-products/?per_page=1`);
...
export default ConditionalFeaturedProducts; 