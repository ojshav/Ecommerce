...
const navLinks = [
  { label: 'HOME', href: '/shop2' },
  { label: 'ABOUT', href: '/shop2/about' },
...
const Header: React.FC = () => {
  ...
  const performSearch = async (query: string) => {
    ...
    try {
      ...
      const response = await shop2ApiService.searchProducts(query, {
        page: 1,
  ...
  const handleSearchResultClick = (product: Product) => {
    setSearch('');
    setShowSearchResults(false);
    navigate(`/shop2/product/${product.product_id}`);
  };
  ...
  const handleSearchSubmit = (e: React.FormEvent) => {
    ...
    if (search.trim()) {
      setShowSearchResults(false);
      navigate(`/shop2-allproductpage?search=${encodeURIComponent(search.trim())}`);
    }
  };
...
<path d="M20.447 20.452H16.893V14.883C16.893 13.555 16.866 11.846 15.041 11.846C13.188 11.846 12.822 13.291 12.822 14.785V20.452H9.351V9H12.694V10.561H12.732C13.297 9.448 14.507 8.207 16.492 8.207C20.523 8.207 20.447 11.348 20.447 13.994V20.452ZM5.337 7.433C4.193 7.433 3.274 6.507 3.274 5.368C3.274 4.23 4.194 3.305 5.337 3.305C6.477 3.305 7.397 4.23 7.397 5.368C7.397 6.507 6.476 7.433 5.337 7.433ZM7.119 20.452H3.555V9H7.119V20.452ZM22.225 0H1.771C0.792 0 0 0.774 0 1.729V22.271C0 23.227 0.792 24 1.771 24H22.222C23.2 24 24 23.227 24 22.271V1.729C24 0.774 23.2 0 22.222 0H22.225Z" fill="#0077B5"/>
        ...
        <div className="flex flex-col md:flex-row max-w-[1440px] mx-auto items-center justify-between px-4 sm:px-8 md:px-14 lg:px-16 py-4 md:py-10 bg-white gap-4 md:gap-0">
          ...
          {/* Desktop Logo */}
          <Link to="/shop2" className="hidden md:block text-[36px] font-playfair font-bold tracking-wide hover:opacity-80 transition-opacity">AOIN</Link>
                        ...
                        >
                          <img
                            ...
                            onError={(e) => {
                              ...
                              target.src = '/assets/shop2/ProductPage/pd1.svg';
                            }}
                          />
