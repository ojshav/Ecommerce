...
const SimilarProducts = () => {
  return (
    <section className="relative w-full max-w-[1428px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-16 lg:ml-20 2xl:pl-10 text-black">
      ...
      <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-[47px] font-normal font-bebas mb-4 sm:mb-6 lg:mb-12 mt-12 sm:mt-16 lg:mt-20">CUSTOMER ALSO LIKED</h2>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 xl:gap-7">
        {customerLikedProducts.map((product) => (
          <div key={product.id} className="relative rounded-xl overflow-hidden">
            <div className="relative">
              <img
                src={product.image}
              ...
              <div className="absolute top-1/2 left-2 sm:left-4 transform -translate-y-1/2 bg-white bg-opacity-50 rounded-full shadow w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 flex items-center justify-center">
                <span className="text-xs sm:text-sm md:text-base">←</span>
              </div>
              <div className="absolute top-1/2 right-2 sm:right-4 transform -translate-y-1/2 bg-white rounded-full shadow w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 flex items-center justify-center">
                <span className="text-xs sm:text-sm md:text-base">→</span>
              ...
              <div className="absolute bottom-0 font-bebas left-0 w-full flex flex-col sm:flex-row justify-between items-center px-2 sm:px-3 lg:px-4 py-2 sm:py-3 bg-opacity-60">
                <button className="bg-black text-white px-6 sm:px-4 lg:px-6 xl:px-10 py-1 sm:py-2 lg:py-3 rounded-full text-xs sm:text-sm font-semibold mb-2 sm:mb-0 sm:mr-2 w-full sm:w-auto">
                  ADD TO CART
                </button>
                <button className="border text-white px-6 sm:px-4 lg:px-6 xl:px-10 py-1 sm:py-2 lg:py-3 rounded-full text-xs sm:text-sm font-semibold w-full sm:w-auto">
                  BUY NOW
                </button>
              </div>
            </div>
